...
//red day
app.get("/redday/:selectday", async (req, res) => {
  const selectday = req.params.selectday; // รับค่าปีจากพารามิเตอร์ URL
  try {
    connection.query(
      "SELECT `h1`,`h2`, `h3`,`h4`,`h5`,`h6`,`h7`,`h8`,`h9`,`h10`,`h11`,`h12`,`h13`,`h14`,`h15`,`h16`,`h17`,`h18`,`h19`,`h20`,`h21`,`h22`,`h23`,`h24` FROM `lineredday_1` WHERE `date`=?;",[selectday],
      (err, results, field) => {
        if (err) {
          console.log(err);
          return res.status(400).send();
...
//green day
app.get("/greenday/:selectday", async (req, res) => {
  const selectday = req.params.selectday; // รับค่าปีจากพารามิเตอร์ URL
  try {
    connection.query(
      "SELECT `h1`,`h2`, `h3`,`h4`,`h5`,`h6`,`h7`,`h8`,`h9`,`h10`,`h11`,`h12`,`h13`,`h14`,`h15`,`h16`,`h17`,`h18`,`h19`,`h20`,`h21`,`h22`,`h23`,`h24` FROM `linegreenday_1` WHERE `date`=?;",[selectday],
      (err, results, field) => {
        if (err) {
          console.log(err);
          return res.status(400).send();
...
//green day
app.get("/typeday/:selectday", async (req, res) => {
  const selectday = req.params.selectday; // รับค่าปีจากพารามิเตอร์ URL
  try {
    connection.query(
      "SELECT `Probe`,`Trojan`,`Bitcoin`,`U2R`,`Web Shell`,`Botnet`,`Brute Force Attack`,`Injection`,`Dropper`,`Ransomware`,`R2L`,`Worm`,`DDoS_DoS`,`Man in the Middle Attacks`,`Data Leak`,`Phishing`,`Website Defacement` FROM `typevongkom` WHERE `date`=?;",[selectday],
      (err, results, field) => {
        if (err) {
          console.log(err);
          return res.status(400).send();
